...
3. **Run the frontend** (from repo root):
   ...
   - **Note:** Coaching whispers use **Gemini Live TTS** (Puck voice) for natural, human-like whisper audio. You should *hear* the whisper as well as see the text. Both whispers and backchannel acknowledgments use the same voice persona. Enable the **webcam checkbox** before starting to see vision-aware coaching.
...
4. **Test script**
   ...
   - **Backchannel:** While you speak, you may hear subtle empathetic backchannels (e.g. "Ok.", "I see.") at low volume — generated by Gemini Live TTS in the same Puck whisper voice to acknowledge you in real time.
...
3. **Backchannel** — While speaking, listen for subtle empathetic backchannels (e.g. "Ok.", "I see.") generated by Gemini Live TTS in the same Puck whisper voice at low volume.
