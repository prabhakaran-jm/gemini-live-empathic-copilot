...
| Component | Role |
|-----------|------|
| **Browser (React/Vite)** | Captures mic audio (PCM16 16kHz), sends base64 chunks + RMS telemetry via WebSocket. Displays tension bar, live transcript, coaching whispers. Plays whisper audio via Web Audio API (Gemini Live PCM24k) or Web Speech API fallback. |
| **Cloud Run (FastAPI)** | Accepts WebSocket at `/ws`. Runs tension scoring loop (RMS, silence, overlap → 0-100). Runs whisper loop with 3 deterministic triggers. Streams audio to Gemini Live for transcription. Calls Gemini Flash for coaching text; when `COACHING_LIVE_AUDIO=1` (default), opens a short-lived Gemini Live TTS session to synthesize whisper audio (PCM24k). |
