...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --concurrency 10 \
  --min-instances 1 \
  --set-env-vars "GOOGLE_CLOUD_PROJECT=${PROJECT_ID},GOOGLE_CLOUD_REGION=${REGION},GEMINI_MODEL=${GEMINI_MODEL:-gemini-live-2.5-flash-native-audio},BARGE_IN_RMS_THRESHOLD=${BARGE_IN_RMS_THRESHOLD:-0.15},TENSION_WHISPER_THRESHOLD=${TENSION_WHISPER_THRESHOLD:-24},COACHING_GROUNDING=${COACHING_GROUNDING:-0},COACHING_LIVE_AUDIO=${COACHING_LIVE_AUDIO:-1},LIVE_BACKCHANNEL=${LIVE_BACKCHANNEL:-1},GEMINI_RECONNECT=${GEMINI_RECONNECT:-1}" \
  "${EXTRA_FLAGS[@]}"
